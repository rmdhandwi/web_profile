...
class Konfigurasi extends BaseController
{
    ...
    public function logo()
    {
        ...
        if ($this->request->getMethod() === 'post' && $this->validate([
            ...
            $namabaru = $avatar->getRandomName(); // Generate random name for file

            if ($avatar->move(FCPATH . 'assets/upload/image/', $namabaru)) {
                // Hapus gambar lama
                ...
                $m_konfigurasi->edit($data);
                $this->session->setFlashdata('sukses', 'Logo telah diperbarui');
            } else {
                $this->session->setFlashdata('error', 'Gagal mengunggah logo.');
            }
    ...
    public function icon()
    {
        ...
        if ($this->request->getMethod() === 'post' && $this->validate([
            ...
            if ($avatar->move(FCPATH . 'assets/upload/image/', $namabaru)) {
                ...
                $m_konfigurasi->edit($data);
                $this->session->setFlashdata('sukses', 'Icon telah diperbarui');
            } else {
                $this->session->setFlashdata('error', 'Gagal mengunggah icon.');
            }
